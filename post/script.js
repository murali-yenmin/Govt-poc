...
// Loop through each element and apply styles based on position
// Loop through each element and apply styles based on position
// Select all elements with the class 'newsp'
...
newspElements.forEach((element, index) => {
  // Determine which 9-element group we are in
  const groupOfNine = Math.floor(index / 9);
  const positionInGroup = index % 9;

  // Apply the pattern based on the group number
  if (groupOfNine % 4 === 0) {
    // Pattern 1-0-0: Set widths for the 1st, 4th, and 7th positions
    if (positionInGroup === 0 || positionInGroup === 3 || positionInGroup === 6) {
      element.style.width = '48%'; // "1" positions in 1-0-0 pattern
    } else {
      element.style.width = '25%'; // "0" positions
    }
  } else if (groupOfNine % 4 === 1) {
    // Pattern 0-1-0: Set widths for the 2nd, 5th, and 8th positions
    if (positionInGroup === 1 || positionInGroup === 4 || positionInGroup === 7) {
      element.style.width = '48%'; // "1" positions in 0-1-0 pattern
    } else {
      element.style.width = '25%'; // "0" positions
    }
  } else if (groupOfNine % 4 === 2) {
    // Pattern 0-0-1: Set widths for the 3rd, 6th, and 9th positions
    if (positionInGroup === 2 || positionInGroup === 5 || positionInGroup === 8) {
      element.style.width = '48%'; // "1" positions in 0-0-1 pattern
    } else {
      element.style.width = '25%'; // "0" positions
    }
  } else if (groupOfNine % 4 === 3) {
    // Repeating the pattern 0-1-0 again (similar to Pattern 2)
    if (positionInGroup === 1 || positionInGroup === 4 || positionInGroup === 7) {
      element.style.width = '48%'; // "1" positions in 0-1-0 pattern
    } else {
      element.style.width = '25%'; // "0" positions
...
// // Arrays with class logic
// const a = [1, 0, 0, 1, 0, 1, 0, 0, 1];
// const b = [0, 1, 0, 0, 1, 0, 0, 1, 0];
// const c = [0, 0, 1, 0, 0, 1, 0, 0, 1];

// // Get all the blog divs
// const blogs = document.querySelectorAll('.newsp');
// const totalBlogs = blogs.length; // Get the total number of divs

// // Function to apply classes based on an array of 1s and 0s
// const applyClasses = (arr, blogs, startIndex) => {
//     arr.forEach((value, index) => {
//         // Only apply classes if the blog exists
//         if (startIndex + index < totalBlogs) {
//             const blog = blogs[startIndex + index];
//             if (value === 1) {
//                 blog.classList.add('w-48');
//                 blog.classList.remove('w-26');
//             } else {
//                 blog.classList.add('w-26');
//                 blog.classList.remove('w-48');
//             }
//         }
//     });
// };

// // Function to loop and apply the logic
// const applyLoopingLogic = () => {
//     let index = 0;

//     // Apply logic for a, b, and c in order
//     [a, b, c].forEach((arr) => {
//         applyClasses(arr, blogs, index);
//         index += 9;
//     });

//     // Apply logic for b, a in reverse order
//     [b, a].forEach((arr) => {
//         applyClasses(arr, blogs, index);
//         index += 9;
//     });
